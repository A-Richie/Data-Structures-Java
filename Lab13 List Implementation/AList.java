...
class AList<T> implements ListInterface<T> {
    ...
    */
    public void reverse()
    {
        if(isEmpty() == false)
        {
            //tail holds the position at the end of the list
            //  (without messing with numberOfEntries variable)
            int tail = numberOfEntries;
            //iterates through array (index goes to middle of array by increasing by 1)
            for(int index = 1; index <= (numberOfEntries / 2); index++)
            {
                T swapEntry = list[tail];
                replace(tail, list[index]);
                replace(index, swapEntry);
                //tail goes closer to middle of array by decreasing by 1
                tail--;
            }
        }
    }
    ...
    */
    public void cycle()
    {
        if(isEmpty() == false)
        {
            T firstEntry = remove(1); // Removes entry but saves to variable
                                        // remove method iterates through to move everything
            add(firstEntry);    //adds first element to end of list
        }        
    }
