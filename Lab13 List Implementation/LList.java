...
class LList<T> implements ListInterface<T> {
    ...
    */
    ...
    public void reverse() {
        if(isEmpty() == false)
        {
            int indexAtEnd = numberOfEntries;
            for(int index = 1; index <= (numberOfEntries / 2); index++)
            {
                //gets information from end of linked list
                T nodeAtEnd = getNodeAt(indexAtEnd).getData();
                
                //does swap with end then swap at beggining
                replace(indexAtEnd, getNodeAt(index).getData());
                replace(index, nodeAtEnd);
                indexAtEnd--;
            }
        }
    }
