...
{
    //Creates an object of type person and adds it to the network/ dictionary
    ...
    public static void display(DictionaryInterface<String, Person> network)
    {
        ...
        {
            System.out.print(keyIterator.next() + "'s friends" + valueIterator.next().toString() + '\n');
        }
            
        System.out.println();
    } // end display
    
    //checks that a key value pair with a specified key is in the dictionary
    public static boolean checkIsPossible(String userhandle, DictionaryInterface<String, Person> network)
    {
        if(network.contains(userhandle) == true)
        {
            return true;
        }
        else
        {
            System.out.println("Error: This profile doesn't exist!");
            return false;
        }
    }
