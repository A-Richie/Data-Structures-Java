...
//
//  Alyssa Richie
//  Graphs - Exercise 1 - Directed Weighted // Final Part 1
//  3/11/2022
//  Team: Team Two's
//
...
//adds an edge with a specific weight for that edge
//returns if the add was sucessful
...
//UNDIRECTED GRAPH IMPLEMENTATION of removeEdge
//Param weight (0.0 for unweighted)
//Param begin & end - Verticies for where the edge would be between
public boolean removeEdge(T begin, T end, double edgeWeight)
{
    ...
    if(result)
        edgeCount--;    //Edgecount decreases by one because the connection was removed
...
{
    boolean found = false;  //return value (variable)

    //gets value of start and end vertex
    VertexInterface<T> beginVertex = vertices.getValue(begin);
    VertexInterface<T> endVertex = vertices.getValue(end);
    ...
    if( (beginVertex != null) && (endVertex != null))
    {
        //neighbors starts at the begin vertex and iterate through neighbors
        Iterator<VertexInterface<T>> neighbors = beginVertex.getNeighborIterator();

        //while end vertex is not found and there is another neighbor keep going
        while(!found && neighbors.hasNext())
        {
            //increment / advance the neighbor value
            VertexInterface<T> nextNeighbor = neighbors.next();

            //compares the values of end vertex and neight -> check they're the same (end vertex is found)
...
//Returns: check if empty graph
public boolean isEmpty() 
{
    return vertices.isEmpty();
}

//Returns: number of entries in graph
...
public QueueInterface<T> getBreadthFirstTraversal(T origin) 
{
    ...
    while(!vertexQueue.isEmpty())
    {
        VertexInterface<T> frontVertex = vertexQueue.dequeue();
        Iterator<VertexInterface<T>> neighbors = frontVertex.getNeighborIterator();
        //checks that the vertex has a neighbor // not a leaf
        while (neighbors.hasNext())
        {
            VertexInterface<T> nextNeighbor = neighbors.next(); //neighbor vertex
            //makes sure the next neighbor wasn't visited
            if(!nextNeighbor.isVisited())
            {
                nextNeighbor.visit();
                traversalOrder.enqueue(nextNeighbor.getLabel());
...
//Remove vertex for undirected graph
