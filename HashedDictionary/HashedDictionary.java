...
/*
*   Hashing - Exercise 2 - HashedDictionary
*   Alyssa Richie
*   2/15/2022
*/
...
public class HashedDictionary<K, V> implements DictionaryInterface<K, V>
{
   ...
   // The dictionary:
    ...
    public V add(K key, V value)
    {
        ...
        {
            V oldValue;     //value to be returned (returns what value gets replaced, if any)

            int index = getHashIndex(key);
            index = probe(index, key); //Probe checks and resolves any collision

            //Checks if index is within the legal range for hashTable
            assert (index >= 0) && (index < hashTable.length);

            //This checks if key is found or not
            if( (hashTable[index] == null) || (hashTable[index].isRemoved()) )
            {
                //The key wans't found so we add a new key and value
                hashTable[index] = new TableEntry<>(key, value);
                numberOfEntries++;
                oldValue = null;
            }
            else
            { //Key is found so we replace the old value with the current value that was passed in
                oldValue = hashTable[index].getValue();
                hashTable[index].setValue(value);
            }

            //checks if table is too big, if it is too big make room through enlargeHashTable method
    ...
    public V remove(K key)
    {
        ...
        if(index != -1)
        {
            //Key is found - so return the value asociated with the key and flag entry as removed
    ...
    public V getValue(K key)
    {
        ...
        //checks if there is a key at an index
        if(index != -1)
        {
            //in the hash table, get the value associated with the key
            result = hashTable[index].getValue(); //Key is found -> return its value
        }

        // result will be null if a key isn't found in dictionary
        return result;
    } // end getValue
